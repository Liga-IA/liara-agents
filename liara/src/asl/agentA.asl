...
+thing(XThing,YThing,dispenser,Parameters)[entity(Myname),source(percept)]: position(XMy,YMy)
...
+roleZone(XZone,YZone)[entity(Myname),source(percept)]: position(XMy,YMy)
...
+goalZone(XZone,YZone)[entity(Myname),source(percept)]: position(XMy,YMy)
	<- 	.print("Perceived a GoalZone at position (",XZone,",",YZone,"). with parameters: ",Parameters,
				". Which is (",XMy+XZone,",",YMy+YZone,") considering my absolute position (",XMy,",",YMy,")"
			  );
